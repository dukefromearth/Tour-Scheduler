...
(* I understand that I shouldn't use mutable data types, but couldn't
...
let open_schedule_csv = Button.create
    ...
    ~command:(fun () ->
      ...
      in (s := schedule sfile;
        Textvariable.set sched_lab ("Schedule set with " ^ (List.hd !s).date ^ " at the head.");
          flush stdout))
    top in
...
let open_employee_csv = Button.create
    ...
    in (e := employees efile;
        Textvariable.set emp_lab ("Employees set with " ^ (List.hd !e).name ^ " at the head.")))
    top in
...
let output_schedule_to_file = Button.create
    ...
    ~command:(fun () ->
      if ((!e != []) || (!s != [])) then
        let sched = (make_and_save_schedule_to_file !e !s)
        in sched;
